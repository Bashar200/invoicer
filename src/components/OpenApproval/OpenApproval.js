...
export default function DataTable() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(`http://192.168.5.213:80/invoice/${selectedIds}/approve`,{'approved_by':'Approver 3'});
        console.log(response.data);
        const status = response.data ? `Invoice ID: ${selectedIds} has been approved` : `Invoice ID: ${selectedIds} is already approved`;
        alert(` ${status}`);
